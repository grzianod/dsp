...
var Images = require(path.join(__dirname, '../service/ImageService.js'));
...
module.exports.getSingleImage = function getSingleImage (req, res, next) {
    ...
    var imageType = mediaType.substring(mediaType.lastIndexOf("/") ).replace('/', '');

    Images.getSingleImage( req.params.filmId, imageType, req.params.imageId, req.user.id)
        .then(function (response) {
            if(imageType === 'json'){
                utils.writeJson(res, response);
            }
